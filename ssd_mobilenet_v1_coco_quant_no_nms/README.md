...
SSD with MobileNet V1 backbone trained on COCO. This variant does **NOT** include Non-Maximum Suppression — it outputs raw detections with overlapping boxes.

You must implement NMS yourself. Useful when you need custom postprocessing logic or want to apply different NMS thresholds.
...
| File | Description |
|------|-------------|
| `ssd_mobilenet_v1_coco_quant_no_nms.tflite` | Standard TFLite model (CPU) |
| `ssd_mobilenet_v1_coco_quant_no_nms_edgetpu.tflite` | Edge TPU compiled model (Coral) |
...
# Load model
interpreter = make_interpreter("ssd_mobilenet_v1_coco_quant_no_nms_edgetpu.tflite")
