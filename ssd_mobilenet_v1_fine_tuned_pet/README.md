...
| File | Description |
|------|-------------|
| `ssd_mobilenet_v1_fine_tuned_pet.tflite` | Standard TFLite model (CPU) |
| `ssd_mobilenet_v1_fine_tuned_pet_edgetpu.tflite` | Edge TPU compiled model (Coral) |

## Labels

The file `pet_labels.txt` maps class IDs to human-readable names.
...
# Load labels
with open("pet_labels.txt", "r") as f:
    labels = [line.strip() for line in f.readlines()]

# Load model
interpreter = make_interpreter("ssd_mobilenet_v1_fine_tuned_pet_edgetpu.tflite")
...
interpreter = tflite.Interpreter(model_path="ssd_mobilenet_v1_fine_tuned_pet.tflite")
interpreter.allocate_tensors()
...
Performance data from the [Coral Models page](https://coral.ai/models/all/).
