...
**Input:** `uint8` tensor of shape `[1, 448, 448, 3]` (batch, height, width, RGB channels), values in `[0, 255]`
...
EfficientDet-Lite2 object detection with 448x448 input. Mid-range variant for 90 COCO categories.

Balances computational cost with detection performance.
...
| Range | `[0, 255]` |
| Color format | RGB |
| Preprocessing | Resize to 448x448, no normalization needed |
...
| File | Description |
|------|-------------|
| `efficientdet_lite2_448_ptq.tflite` | Standard TFLite model (CPU) |
| `efficientdet_lite2_448_ptq_edgetpu.tflite` | Edge TPU compiled model (Coral) |
...
# Load model
interpreter = make_interpreter("efficientdet_lite2_448_ptq_edgetpu.tflite")
...
interpreter = tflite.Interpreter(model_path="efficientdet_lite2_448_ptq.tflite")
interpreter.allocate_tensors()
...
|--------|-------|
| mAP (COCO 2017) | 36.0% |
| Edge TPU Latency | 104.6 ms |
| Model Size (Edge TPU) | 11 MB |
