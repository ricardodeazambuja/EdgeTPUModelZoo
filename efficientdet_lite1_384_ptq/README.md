...
**Input:** `uint8` tensor of shape `[1, 384, 384, 3]` (batch, height, width, RGB channels), values in `[0, 255]`
...
EfficientDet-Lite1 object detection model. Slightly larger than Lite0 with improved accuracy for 90 COCO categories.

Good balance between speed and accuracy for edge deployment.
...
| Range | `[0, 255]` |
| Color format | RGB |
| Preprocessing | Resize to 384x384, no normalization needed |
...
| File | Description |
|------|-------------|
| `efficientdet_lite1_384_ptq.tflite` | Standard TFLite model (CPU) |
| `efficientdet_lite1_384_ptq_edgetpu.tflite` | Edge TPU compiled model (Coral) |
...
# Load model
interpreter = make_interpreter("efficientdet_lite1_384_ptq_edgetpu.tflite")
...
interpreter = tflite.Interpreter(model_path="efficientdet_lite1_384_ptq.tflite")
interpreter.allocate_tensors()
...
|--------|-------|
| mAP (COCO 2017) | 34.3% |
| Edge TPU Latency | 56.3 ms |
| Model Size (Edge TPU) | 7.7 MB |
