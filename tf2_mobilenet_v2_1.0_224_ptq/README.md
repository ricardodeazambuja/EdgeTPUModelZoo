...
| File | Description |
|------|-------------|
| `tf2_mobilenet_v2_1.0_224_ptq.tflite` | Standard TFLite model (CPU) |
| `tf2_mobilenet_v2_1.0_224_ptq_edgetpu.tflite` | Edge TPU compiled model (Coral) |
...
# Load model
interpreter = make_interpreter("tf2_mobilenet_v2_1.0_224_ptq_edgetpu.tflite")
...
interpreter = tflite.Interpreter(model_path="tf2_mobilenet_v2_1.0_224_ptq.tflite")
interpreter.allocate_tensors()
...
| Top-5 Accuracy (ImageNet) | 91.8% |
| Edge TPU Latency | 3.0 ms |
| Model Size (Edge TPU) | 4.3 MB |
