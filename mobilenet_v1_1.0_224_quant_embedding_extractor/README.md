...
**Output:** `uint8` tensor of shape `[1, 1, 1, 1024]` — 1024-dimensional feature embedding vector
**Dataset:** ImageNet (pre-trained)
**Quantization:** Full integer (uint8 input and output)

## Description

Feature extraction variant of MobileNet V1 (alpha=1.0, 224x224). Outputs a feature embedding vector instead of classification scores.

Removes the final classification layer to output feature embeddings. Use for transfer learning, similarity search, and on-device learning via weight imprinting.
...
| File | Description |
|------|-------------|
| `mobilenet_v1_1.0_224_quant_embedding_extractor.tflite` | Standard TFLite model (CPU) |
| `mobilenet_v1_1.0_224_quant_embedding_extractor_edgetpu.tflite` | Edge TPU compiled model (Coral) |
...
# Load model
interpreter = make_interpreter("mobilenet_v1_1.0_224_quant_embedding_extractor_edgetpu.tflite")
