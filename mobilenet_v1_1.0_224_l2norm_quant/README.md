# MobileNet V1 with L2 Normalization
...
MobileNet V1 (alpha=1.0, 224x224) with L2 normalization on the output. 1000 ImageNet categories.

The L2-normalized output produces unit-length vectors suitable for similarity-based tasks and metric learning.
...
| File | Description |
|------|-------------|
| `mobilenet_v1_1.0_224_l2norm_quant.tflite` | Standard TFLite model (CPU) |
| `mobilenet_v1_1.0_224_l2norm_quant_edgetpu.tflite` | Edge TPU compiled model (Coral) |
...
# Load model
interpreter = make_interpreter("mobilenet_v1_1.0_224_l2norm_quant_edgetpu.tflite")
...
interpreter = tflite.Interpreter(model_path="mobilenet_v1_1.0_224_l2norm_quant.tflite")
interpreter.allocate_tensors()
