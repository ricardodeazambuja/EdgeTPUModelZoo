...
Inception V3 image classification. 1000 ImageNet categories.

Uses label smoothing, factorized 7x7 convolutions, and auxiliary classifiers. Higher accuracy than V1/V2 but requires more computation.
...
| File | Description |
|------|-------------|
| `inception_v3_299_quant.tflite` | Standard TFLite model (CPU) |
| `inception_v3_299_quant_edgetpu.tflite` | Edge TPU compiled model (Coral) |
...
# Load model
interpreter = make_interpreter("inception_v3_299_quant_edgetpu.tflite")
...
interpreter = tflite.Interpreter(model_path="inception_v3_299_quant.tflite")
interpreter.allocate_tensors()
...
| Metric | Value |
|--------|-------|
| Top-1 Accuracy (ImageNet) | 79.9% |
| Top-5 Accuracy (ImageNet) | 95.7% |
| Edge TPU Latency | 42.8 ms |
| Model Size (Edge TPU) | 25 MB |
