...
SSDLite with MobileDet backbone (NAS-optimized) for object detection on 90 COCO categories. Uses quantization-aware training (QAT) for minimal accuracy loss from quantization. Includes NMS postprocessing.

MobileDet backbone was found via neural architecture search. QAT provides better quantized accuracy than post-training quantization.
...
Where `N` is the maximum number of detections (100).
...
| File | Description |
|------|-------------|
| `ssdlite_mobiledet_coco_qat_postprocess.tflite` | Standard TFLite model (CPU) |
| `ssdlite_mobiledet_coco_qat_postprocess_edgetpu.tflite` | Edge TPU compiled model (Coral) |
...
# Load model
interpreter = make_interpreter("ssdlite_mobiledet_coco_qat_postprocess_edgetpu.tflite")
...
interpreter = tflite.Interpreter(model_path="ssdlite_mobiledet_coco_qat_postprocess.tflite")
interpreter.allocate_tensors()
...
|--------|-------|
| mAP (COCO) | 32.9% |
| Edge TPU Latency | 9.1 ms |
| Model Size (Edge TPU) | 5.2 MB |

Accuracy and latency from the [Coral Models page](https://coral.ai/models/all/).

## References

- Xiong, Y. et al., "MobileDets: Searching for Object Detection Architectures for Mobile Accelerators" ([arXiv:2004.14525](https://arxiv.org/abs/2004.14525))
