...
DeepLabV3 semantic segmentation with full MobileNetV2 backbone. Classifies each pixel into 21 Pascal VOC categories: background, aeroplane, bicycle, bird, boat, bottle, bus, car, cat, chair, cow, dining table, dog, horse, motorbike, person, potted plant, sheep, sofa, train, TV/monitor.

Full depth multiplier (1.0) version offers better accuracy than the DM=0.5 variant.
...
| File | Description |
|------|-------------|
| `deeplabv3_mnv2_pascal_quant.tflite` | Standard TFLite model (CPU) |
| `deeplabv3_mnv2_pascal_quant_edgetpu.tflite` | Edge TPU compiled model (Coral) |
...
interpreter = tflite.Interpreter(model_path="deeplabv3_mnv2_pascal_quant.tflite")
interpreter.allocate_tensors()
...
| Metric | Value |
|--------|-------|
| Edge TPU Latency | 43.0 ms |
| Model Size (Edge TPU) | 2.8 MB |
