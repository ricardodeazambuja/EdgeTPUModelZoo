...
Inception V2 image classification with batch normalization. 1000 ImageNet categories.

Improves upon V1 with batch normalization and factorized convolutions.
...
| File | Description |
|------|-------------|
| `inception_v2_224_quant.tflite` | Standard TFLite model (CPU) |
| `inception_v2_224_quant_edgetpu.tflite` | Edge TPU compiled model (Coral) |
...
# Load model
interpreter = make_interpreter("inception_v2_224_quant_edgetpu.tflite")
...
interpreter = tflite.Interpreter(model_path="inception_v2_224_quant.tflite")
interpreter.allocate_tensors()
