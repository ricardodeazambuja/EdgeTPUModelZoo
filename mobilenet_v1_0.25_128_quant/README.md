# MobileNet V1 (alpha=0.25, 128x128)
...
| Range | `[0, 255]` |
| Color format | RGB |
| Preprocessing | Resize to 128x128, no normalization needed (quantized model) |
...
| File | Description |
|------|-------------|
| `mobilenet_v1_0.25_128_quant.tflite` | Standard TFLite model (CPU) |
| `mobilenet_v1_0.25_128_quant_edgetpu.tflite` | Edge TPU compiled model (Coral) |
...
# Load model
interpreter = make_interpreter("mobilenet_v1_0.25_128_quant_edgetpu.tflite")
interpreter.allocate_tensors()

# Prepare image: resize to 128x128 and convert to uint8 RGB
image = Image.open("your_image.jpg").convert("RGB").resize((128, 128))
...
interpreter = tflite.Interpreter(model_path="mobilenet_v1_0.25_128_quant.tflite")
interpreter.allocate_tensors()
...
| Metric | Value |
|--------|-------|
| Top-1 Accuracy (ImageNet) | 40.8% |
| Top-5 Accuracy (ImageNet) | 67.2% |
| Edge TPU Latency | 0.9 ms |
| Model Size (Edge TPU) | 0.8 MB |
