# SSD MobileNet V1 COCO (with Postprocess)
...
SSD with MobileNet V1 backbone trained on COCO. Includes built-in NMS postprocessing for ready-to-use detection results across 90 categories.

Ready-to-use: outputs filtered detections with bounding boxes, class IDs, and confidence scores.
...
| File | Description |
|------|-------------|
| `ssd_mobilenet_v1_coco_quant_postprocess.tflite` | Standard TFLite model (CPU) |
| `ssd_mobilenet_v1_coco_quant_postprocess_edgetpu.tflite` | Edge TPU compiled model (Coral) |
...
# Load model
interpreter = make_interpreter("ssd_mobilenet_v1_coco_quant_postprocess_edgetpu.tflite")
