# MobileNet V1 (alpha=0.5, 160x160)

**Task:** Image Classification
**Input:** `uint8` tensor of shape `[1, 160, 160, 3]` (batch, height, width, RGB channels), values in `[0, 255]`
...
MobileNet V1 with width multiplier 0.5 and 160x160 input. 1000 ImageNet categories.

Mid-range compact variant with half the channels of the full model.
...
| Range | `[0, 255]` |
| Color format | RGB |
| Preprocessing | Resize to 160x160, no normalization needed (quantized model) |
...
| File | Description |
|------|-------------|
| `mobilenet_v1_0.5_160_quant.tflite` | Standard TFLite model (CPU) |
| `mobilenet_v1_0.5_160_quant_edgetpu.tflite` | Edge TPU compiled model (Coral) |
...
# Load model
interpreter = make_interpreter("mobilenet_v1_0.5_160_quant_edgetpu.tflite")
interpreter.allocate_tensors()

# Prepare image: resize to 160x160 and convert to uint8 RGB
image = Image.open("your_image.jpg").convert("RGB").resize((160, 160))
...
interpreter = tflite.Interpreter(model_path="mobilenet_v1_0.5_160_quant.tflite")
interpreter.allocate_tensors()
...
| Metric | Value |
|--------|-------|
| Top-1 Accuracy (ImageNet) | 63.7% |
| Top-5 Accuracy (ImageNet) | 83.4% |
| Edge TPU Latency | 1.4 ms |
| Model Size (Edge TPU) | 1.7 MB |
