# TF2 SSD MobileNet V1 FPN (640x640)

**Task:** Object Detection (with built-in NMS postprocessing)
**Input:** `uint8` tensor of shape `[1, 640, 640, 3]` (batch, height, width, RGB channels), values in `[0, 255]`
...
| Range | `[0, 255]` |
| Color format | RGB |
| Preprocessing | Resize to 640x640, no normalization needed |
...
| File | Description |
|------|-------------|
| `tf2_ssd_mobilenet_v1_fpn_640x640_coco17_ptq.tflite` | Standard TFLite model (CPU) |
| `tf2_ssd_mobilenet_v1_fpn_640x640_coco17_ptq_edgetpu.tflite` | Edge TPU compiled model (Coral) |
...
# Load model
interpreter = make_interpreter("tf2_ssd_mobilenet_v1_fpn_640x640_coco17_ptq_edgetpu.tflite")
...
interpreter = tflite.Interpreter(model_path="tf2_ssd_mobilenet_v1_fpn_640x640_coco17_ptq.tflite")
interpreter.allocate_tensors()
...
|--------|-------|
| mAP (COCO 2017) | 31.1% |
| Edge TPU Latency | 229.4 ms |
| Model Size (Edge TPU) | 38 MB |
...
- Lin, T.-Y. et al., "Feature Pyramid Networks for Object Detection" ([arXiv:1612.03144](https://arxiv.org/abs/1612.03144))
