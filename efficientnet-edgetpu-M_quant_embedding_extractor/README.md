# EfficientNet-EdgeTPU Medium Embedding Extractor
...
Feature extraction variant of EfficientNet-EdgeTPU Medium. Outputs a feature embedding vector for transfer learning and similarity applications.

Use this to extract feature vectors for image retrieval, clustering, or on-device transfer learning.
...
| Range | `[0, 255]` |
| Color format | RGB |
| Preprocessing | Resize to 240x240, no normalization needed |
...
| File | Description |
|------|-------------|
| `efficientnet-edgetpu-M_quant_embedding_extractor.tflite` | Standard TFLite model (CPU) |
| `efficientnet-edgetpu-M_quant_embedding_extractor_edgetpu.tflite` | Edge TPU compiled model (Coral) |
...
def get_embedding(interpreter, image_path, width=240, height=240):
...
# Load model
interpreter = make_interpreter("efficientnet-edgetpu-M_quant_embedding_extractor_edgetpu.tflite")
