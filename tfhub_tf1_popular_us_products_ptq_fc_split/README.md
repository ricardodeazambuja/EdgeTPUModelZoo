# TF Hub Popular US Products (FC Split)
...
Product recognition model with the fully-connected layer split for Edge TPU co-compilation. Same functionality as the standard variant.

The FC split architecture enables co-compilation with other models on the same Edge TPU, allowing multiple models to share TPU resources.
...
| File | Description |
|------|-------------|
| `tfhub_tf1_popular_us_products_ptq_fc_split.tflite` | Standard TFLite model (CPU) |
| `tfhub_tf1_popular_us_products_ptq_fc_split_edgetpu.tflite` | Edge TPU compiled model (Coral) |
...
# Load model
interpreter = make_interpreter("tfhub_tf1_popular_us_products_ptq_fc_split_edgetpu.tflite")
...
interpreter = tflite.Interpreter(model_path="tfhub_tf1_popular_us_products_ptq_fc_split.tflite")
interpreter.allocate_tensors()
